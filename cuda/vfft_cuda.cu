...
// Inverse path for STEP2A velocity reconstruction. STEP2A prepare has already
// zeroed high kx, so the z inverse is pruned to the columns used by C2R.
void vfft_step2a_inverse_uc_needed_to_ur_full(DnsDeviceState *S)
{
    ...
    for (int c = 0; c < 2; ++c) {
        ...
        cufftResult r = cufftExecC2C(S->plan_full_c2c_z, comp, comp, CUFFT_INVERSE);
        if (r != CUFFT_SUCCESS) {
            printf("vfft_step2a_inverse: cufftExecC2C z failed, code=%d\n", r);
            return;
        }
    }
    ...
    if (r != CUFFT_SUCCESS) {
        printf("vfft_step2a_inverse: cufftExecC2R x failed, code=%d\n", r);
    }
}
